...
void layer_norm_raw_fp64_sdma_ssr_frep_omp(
    ...
    for (size_t b1 = 0; b1 < B; b1 += batch_buf_size) {
        ...
        if (snrt_is_compute_core()) {
            snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_3D, &tmp_buf0[N * b2_len_thr * tid]);
            snrt_ssr_read(SNRT_SSR_DM1, SNRT_SSR_3D, gam_buf);
            snrt_ssr_write(SNRT_SSR_DM2, SNRT_SSR_3D, &tmp_buf0[N * b2_len_thr * tid]);

            snrt_ssr_enable();
            for (size_t b2 = b2_len_thr * tid; b2 < b2_len_thr * (tid + 1); b2 += 1) {
                size_t b = b1 + b2;
